...
export class ServiceUpdater {
  ...
  constructor(
    ...
    readonly existingTaskDef: EcsTaskDefinition,
  ...
  buildServiceUpdate(): Observable<EcsServicePatchResult> {
    const ImagePrev = this.existingTaskDef.ContainerDefinitions.find(cnt => cnt.Name === 'default')?.Image
    ...
    const desiredTaskDef = new EcsTaskDefinition(this.existingTaskDef)
